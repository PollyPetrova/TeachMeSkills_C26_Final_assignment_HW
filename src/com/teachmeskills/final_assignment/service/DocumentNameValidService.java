...
// создаем объекты, где лежат пути к папкам
        File validCheck = new File(Constant.PATH_VALID_CHECK);
        File validInvoice = new File(Constant.PATH_VALID_INVOICE);
        File validOrder = new File(Constant.PATH_VALID_ORDER);
        File invalidDocName = new File(Constant.PATH_INVALID_DOC);
...
//проверяем на валидность чека, вызываем метод перемещения
        if (doc.getName().matches(Constant.REGEXP_DOC_CHECK)) {
            try {
...
//проверяем на валидность инвойса, вызываем метод перемещения
        if(doc.getName().matches(Constant.REGEXP_DOC_INVOICE)){
            try {
...
//проверяем на валидность ордера, вызываем метод перемещения
        if (doc.getName().matches(Constant.REGEXP_DOC_ORDER)) {
            try {
        ...
        if (!doc.getName().matches(Constant.REGEXP_DOC_CHECK)
                && !doc.getName().matches(Constant.REGEXP_DOC_ORDER)
                && !doc.getName().matches(Constant.REGEXP_DOC_INVOICE)) {
            try {
                DocumentMoveService.moveDoc(doc, invalidDocName);
